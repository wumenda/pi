...
- `packages/app-server/test/`：boot / config / llm / sessions / runtime / services / mcp-config / mcp-host / tool-events / token / users / http-ui-resources / http-mcp-tools / http-files / http-static / crash-recovery（重启后遗留挂起 run 的自动收尾，见核心机制「崩溃恢复」）。
