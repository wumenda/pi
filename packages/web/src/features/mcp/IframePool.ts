import { type IframeInstance, type SkillReadContext, SOLO_GROUP_KEY } from "./types.ts";

/** 池容量建议上限（建议值，不强制） */
export const IFRAME_POOL_CAPACITY = 5;

/**
 * iframe 池复合键：`<resourceUri>#<group>`。
 * 有 serverId 时前缀 `<serverId>|`——同一 resourceUri 归属不同 MCP server
 * 是池内不同条目，互不复用、隔离契约。
 * `instanceNo > 0` 为该资源/分组下并发执行派生的独立实例，key 追加 `#i<no>`
 * （基实例 instanceNo=0 保持旧格式）。查找一律按实例字段，不解析 key。
 */
...
export interface IframeAcquireMeta {
	...
	/** tool tab 展示名（仅首次创建时生效） */
	title?: string;
	/** tool 标识符（归属判定用，与展示名 title 解耦） */
	toolName?: string;
	/** 归属分组（skill 实例键 `<name>#<instanceId>` 或 SOLO_GROUP_KEY；仅首次创建时生效） */
	group?: string;
	/**
	 * 归属判定的上下文：tool 执行时最近一次读取的 Skill 实例
	 * （name + instanceId）。store 层用它按「上下文时序」解析归属，不写入池实例；
	 * 不属 IframePool 纯池逻辑，仅随 IframeAcquireMeta 透传。
	 */
...
export interface IframePoolEvent {
	...
	/** 实例键（复合键 `<resourceUri>#<group>`，审计口径） */
	resourceUri: string;
	group?: string;
...
/**
 * iframe 池（独立类；DOM 与池策略不进组件）：
 * - key = 复合键 `<ui.resourceUri>#<group>`——同一 resourceUri 在不同分组
 *   （Skill 实例 / 单独调用）下是独立条目，互不复用；同一时刻只有一个激活实例；
 * - acquire 未命中创建、命中复用（element 引用不变）；
 * - 容量超限时按 LRU 淘汰最久未使用的挂起实例（激活实例不参与淘汰）。
 */
export class IframePool {
	entries = new Map<string, PoolEntry>();
	activeUri: string | null = null;
	seq = 0;
	/** tool 执行标识 → 其绑定的实例 key（同一执行的所有状态变化/进度回原实例） */
	callToKey = new Map<string, string>();
	/** 生命周期事件回调（store 层接线；本模块保持无 store 依赖） */
	onEvent?: (event: IframePoolEvent) => void;
	readonly capacity: number;

	constructor(capacity: number = IFRAME_POOL_CAPACITY) {
		this.capacity = capacity;
	}

	emit(action: IframePoolEvent["action"], key: string, group?: string): void {
		this.onEvent?.({
			action,
			resourceUri: key,
	 ...
	 *  pipeline 在 acquire 前据此预判「本次是否新建实例」，决定冷启动补推。 */
	...
	/**
	 * 取得（或创建）resourceUri 在 group 下的 iframe 实例（复合键 + 实例隔离）：
	...
	acquire(resourceUri: string, meta: IframeAcquireMeta = {}): IframeInstance {
		...
		if (target === null) {
			...
			const entry: PoolEntry = {
				...
				element: createSandboxIframe(meta.permissions),
				boundToolCalls: [],
		...
		// 聚焦语义：每次 acquire 都激活目标实例（completed 也聚焦——恢复查看）；
		// 并发隔离不依赖聚焦，由「执行→实例」通道隔离保证。
	...
	updateActive(entry: PoolEntry, callId: string | undefined, active: boolean | undefined): void {
	...
	nextInstanceNo(resourceUri: string, group: string, serverId?: string): number {
		let maxNo = -1;
		for (const entry of this.entries.values()) {
			if (entry.resourceUri !== resourceUri || entry.group !== group) continue;
			if (entry.serverId !== serverId) continue;
